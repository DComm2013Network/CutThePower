/**
 * Defines all functions necessary for updating the client's game information.
 * 
 * The client update system retrieve updates received by the network module and
...
void client_update_system(World *world) {
	if(!player_table)
	{
		...
		memset(player_table, MAX_ENTITIES, MAX_PLAYERS * sizeof(unsigned int)); 
	}
	void* packet;
	uint32_t type;
