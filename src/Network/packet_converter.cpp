/*	
 * packet_converter.c - A simple packet encapsulation/decapsulation library
 *	
 * Designed by Clark Allenby & Shane Spoor
 * Written by Clark Allenby
 *	
 * For starters, this library will convert a PKT_POS_UPDATE and a
 * PKT_ALL_POS_UPDATE to a more network-efficient structure. The total benefit
 ...
 *		PKT_POS_UPDATE encapsulate_pos_update(PKT_OLD_POS_UPDATE old_pkt);
 *		PKT_OLD_POS_UPDATE decapsulate_pos_update(PKT_POS_UPDATE pkt);
 *		PKT_ALL_POS_UPDATE encapsulate_all_pos_update(PKT_OLD_ALL_POS_UPDATE old_pkt);
 *		PKT_OLD_ALL_POS_UPDATE decapsulate_all_pos_update(PKT_ALL_POS_UPDATE pkt);
 *	
 ...
 *
 * @param old_pkt Pointer to the PKT_OLD_POS_UPDATE packet to be encapsulated. Must be valid.
 * @return Pointer to the encapsulated PKT_POS_UPDATE. Returns NULL if the packet couldn't be 
 *         allocated.
 *
...
PKT_NEW_POS_UPDATE *encapsulate_pos_update(PKT_POS_UPDATE *old_pkt) {
	PKT_NEW_POS_UPDATE *pkt = malloc(sizeof(PKT_NEW_POS_UPDATE));
...
PKT_POS_UPDATE *decapsulate_pos_update(PKT_NEW_POS_UPDATE *pkt) {
	PKT_POS_UPDATE *old_pkt = malloc(sizeof(PKT_POS_UPDATE));
...
PKT_NEW_ALL_POS_UPDATE *encapsulate_all_pos_update(PKT_ALL_POS_UPDATE *old_pkt) {
	PKT_NEW_ALL_POS_UPDATE *pkt = malloc(sizeof (PKT_NEW_ALL_POS_UPDATE));
	int i, j;
 ...
 *            </ul>
 *
 * @param pkt Pointer to the PKT_ALL_POS_UPDATE packet to be decapsulated. Must be valid.
 * @return Pointer to the decapsulated PKT_OLD_ALL_POS_UPDATE. This may contain incorrect
 *         or impossible values (according to game rules). Returns NULL if the packet
 *         couldn't be allocated.
 *
 * @designer Clark Allenby
...
PKT_ALL_POS_UPDATE *decapsulate_all_pos_update(PKT_NEW_ALL_POS_UPDATE *pkt) {
	...
	int i, j;
	uint32_t temp, shift1, shift2, and_mask;
