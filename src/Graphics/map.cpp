	...
	if (fscanf(fp_map, "%d", &entity_count) == 1) {
		...
		for(i = 0; i < entity_count; i++) {
			...
			if (strcmp(entity_type, "stair") == 0) { //stair
				fscanf(fp_map, "%d %d %d %d %d", &x, &y, &tx, &ty, &tlevel);
				create_stair(world, tlevel, tx * TILE_WIDTH, ty * TILE_HEIGHT, x * TILE_HEIGHT, y * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT, level);
			}
