...
void handle_entity_collision(CollisionData data, World * world, int curEntityID, unsigned int player_entity);
...
void handle_entity_collision(CollisionData data, World * world, int curEntityID, unsigned int player_entity) {
	...
	case COLLISION_STAIR:
		world->player[player_entity].tilez = rand() % 3 + 1;
		break;
		...
		if (IN_THIS_COMPONENT(world->mask[entity], CONTROLLABLE_MASK)) {
			...
			if (controllable->active == true) {
				...
				if (IN_THIS_COMPONENT(world->mask[entity], COLLISION_MASK)) {
					...
					handle_entity_collision(data, world, entity, player_entity);
